        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button("Ok", GUILayout.Width(150)))
            {
                if (sourceFile != targetFile)
                {
                    ...
                    // Check Existence of the preview image
                    string sourcePreviewFullPath = sourceFullPath.Replace($"/{Path.GetFileName(sourceFullPath)}" , $"/{Path.GetFileNameWithoutExtension(sourceFullPath)}.png");

                    if (AssetDatabase.LoadAssetAtPath<Texture>(sourcePreviewFullPath) != null)
                    {
                        AssetDatabase.RenameAsset(sourcePreviewFullPath, targetFile);
                    }
